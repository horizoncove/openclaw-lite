...
> **PACK_CORE P2.3** · MiniMax / Trae 优先读 **MVP** + **UI** 篇章  
> 当前只要求交付：购 T → 发悬赏 → 接单 → 平台托管收付 T  
> UI 不得照搬 Web3「Connect Wallet / 智能合约」叙事

## 口令

客户买 Token → 发悬赏 → 供应商接单 → 平台冻结客户 T → 验收 → 平台付给供应商 T  
不做：兑法币、出海、网关、监督大盘、互转挂单、链上钱包连接

**UI 纠偏摘要（详文见下方【UI 设计】或仓库 `docs/UI_DESIGN.md`）：**

- 登录用机构账号，禁止 Connect Wallet  
- 单位用 **T**；品类用微短剧履约（译制/配音/发行…）  
- 流程四步：买 → 发 → **确认托管** → 验收放款（平台账本，非智能合约）  
...
> 完整版：`docs/UI_DESIGN.md` UI-1.0  
> 对照外部 TaskMatch 深色营销稿优化：保留「购→发→大厅→验收」骨架，去掉 Web3 隐喻。

## 设计北极星

用户第一眼应感到：**找对人、谈妥有托管、做完能结清**——不是连钱包炒积分。

## MVP 路由

`/login` · `/app` · `/app/wallet` · `/app/bounties` · `/app/bounties/new` · `/app/bounties/:id` · `/app/orders` · `/app/orders/:id`
...
| 大厅 | 品类/预算 T/机构/应征数 |
| 详情 | 应征 → 客户确认 → **冻结反馈** |
| 订单 | 时间线托管→履约→验收→放款；结算单 |

## 文案禁词

去中心化、上链、智能合约自动执行、Connect Wallet、二级市场、市值、流通盘、TKC、Gas

## 验收（UI）

全站无上述禁词；确认后有冻结反馈；品类短剧化；单位 T；MVP 导航无监督/回收/出海/网关。
...
> UI 约束：[UI_DESIGN.md](./UI_DESIGN.md) UI-1.0（对照 TaskMatch 参考稿纠偏）  
...
> **税务：** Token 不单独完税；跟 ¥ 合同与发票走——见 [TAX_AND_SETTLEMENT.md](./TAX_AND_SETTLEMENT.md)（平台给客户开票，供应商给平台开票）。
...
| 工作台（下一件待办） | 按角色 |
| 我的钱包（余额、购 T、流水、开票入口可占位） | 客户 / 供应商 |
| 发布悬赏（短剧履约品类） | 客户 |
| 悬赏大厅（卡片：品类、预算 T、机构、应征数） | 供应商（客户可看） |
| 悬赏详情（应征 / **确认合作→冻结反馈**） | 双方 |
| 我的订单 / 订单详情（时间线、验收、结算单） | 双方 |

可选：工作台只放「待确认接单 / 待验收」两条入口。  
**完整 IA、文案禁词、四步托管示意：** [UI_DESIGN.md](./UI_DESIGN.md)。

### 5.1 UI 必守（相对 Web3 参考稿）

| 必守 | 说明 |
|------|------|
| 单位用 **T** | 不用 TKC / 链上符号 |
| 确认后显式「已冻结」 | M6 不可静默成功 |
| 流程文案四步 | 买 → 发 → **确认托管** → 验收放款（勿写成智能合约自动放款） |
| 品类短剧化 | 译制/配音/发行等，非开发/设计/写作/营销通用外包 |
| 无市值指标 | 可用「托管中订单 / 活跃供应商」；禁止「Token 流通盘 ¥」 |
| MVP 导航不含 | 监督大盘、兑 ¥、出海、算力网关 |
...
**外发实现方（MiniMax/Trae）：只要求交付本文 M1–M10 + §5.1 UI 必守。**  
完整 R2/T2 作背景，不得用卫星功能替代本闭环；不得照搬 TaskMatch「连钱包/智能合约」叙事。

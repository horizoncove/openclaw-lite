...
> 日期：2026-07-25  
> 状态：**需求主入口（整合）**  
> **当前必达实现范围：** [MVP.md](./MVP.md) **MVP-1.1**（购 T→发悬赏→接单→平台收付 T）  
> UI 约束：[UI_DESIGN.md](./UI_DESIGN.md) UI-1.0  
...
| 读者 | 读哪些 |
|------|--------|
| 商务 / 联盟 / MiniMax·Trae | §1–§4、§8 · [MVP.md](./MVP.md) · [UI_DESIGN.md](./UI_DESIGN.md) |
| 产品 | 全文 + UI_DESIGN |
...
| ID | 要求 | 优先级 |
|----|------|--------|
| R-UI-01 | 产品叙事为「机构会员协作 / 平台积分托管」，禁止去中心化、智能合约自动放款、Connect Wallet | P0 |
...
| R-UI-08 | MVP 主导航不含监督/回收/出海/网关算力 | P0 |
---
...
必做：M1–M10（登录、购 T、发悬赏、大厅、接单、确认冻结、订单状态、验收放款、流水、禁互转）+ **R-UI-01～06/08**。  
不做：回收兑 ¥、出海、网关/算力、监督大盘、账期、评价、Web3 钱包连接。

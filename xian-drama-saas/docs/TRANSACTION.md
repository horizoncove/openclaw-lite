...
> 代码基线：v1.7+（托管 escrow + 阶段 phase + 双边确认）
...
| 逻辑 | 文件 |
|------|------|
| 阶段/托管/确认/结算 | `src/utils/dealLoop.ts`、`server/dealLoop.mjs` |
| API | `POST /alliance/deals/close`、`/confirm`、`/consume`、`/settle` |
| 讲解页 | `/alliance/console/loop` 交易本质区 |
| 会员感知 | `/alliance/member/deals` 可用/锁定余额 |

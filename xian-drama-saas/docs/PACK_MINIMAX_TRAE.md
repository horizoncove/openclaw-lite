...
1. **撮合适配保障** — 人对得上：发布 → 应征 → 确认  
2. **信任保障** — 谈妥算数：订单 → T 托管 → 验收放款（+ 争议可裁决）

口令：**先立两柱，再挂卫星；卫星不得反向定义产品。**  
详见合集首篇 `CORE_VALUE_MATCHING`。
...
| 必须做 | 禁止做 |
|--------|--------|
| 适配×信任闭环先于一切炫技 | Token 挂单、会员互兑、浮动汇率 UI |
...
2. **结算拍板（必守）** ← `DECISION_TOKEN_SETTLEMENT.md`
3. **需求规格总册** ← `REQUIREMENTS_SPEC.md`
4. **SaaS 技术架构总册** ← `SAAS_ARCHITECTURE.md`
5. **产品功能规格（界面/字段）** ← `PRD.md`
6. **验收放行清单** ← `ACCEPTANCE.md`
7. **现行 HTTP 契约（实现对照）** ← `API_CONTRACT.md`
8. **现状 vs 目标边界** ← `P1_BOUNDARY.md`
...
**CORE_VALUE C1.1 北极星 > D1.3 结算 > 需求总册 > PRD > 架构 > 现行 API_CONTRACT**
...
```
核  适配保障 × 信任保障：发布→应征→确认→托管→放款
卫星 工作台/钱包/项目/网关/算力/出海/回收/通知/报表
禁  挂单·互兑·兑换所·卫星反客为主
```

前端主路径：`/app/login` → `/app/workspace`  
技术栈：React+Vite · Node/Express · 目标 PostgreSQL。
...
> 源文件：`docs/CORE_VALUE_MATCHING.md`
...
P1.1 · `cursor/overseas-drama-saas-8928`

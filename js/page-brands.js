(function () {
  ...
  function renderFilters() {
    ...
    const zoneWrap = document.getElementById('zone-filter');
    if (!tierWrap || !catWrap || !zoneWrap) return;
    ...
    const zones = ['全部', ...new Set(BRANDS.map((b) => b.macroLabel).filter(Boolean))];
    zoneWrap.innerHTML = zones
      .map((z) => `<span class="badge ${z === '全部' ? 'lime active' : 'ink-outline'}" data-zone="${z}">${z}</span>`)
      .join('');
    ...
    zoneWrap.querySelectorAll('.badge').forEach((badge) => {
      badge.addEventListener('click', () => {
        zoneWrap.querySelectorAll('.badge').forEach((b) => b.classList.remove('active', 'lime'));
        badge.classList.add('active', 'lime');
        activeZone = badge.dataset.zone;
        applyFilters();
      });
    });
  }
  ...
  function renderBrands() {
    ...
    grid.innerHTML = BRANDS.map((b) => `
      <article class="card card-vert brand-card" data-tier="${b.tier}" data-category="${b.cat}" data-zone="${b.macroLabel || ''}" data-search="${[b.name, b.cat, b.subcat, b.probDesc, b.tier, b.zone, b.macroLabel, b.building].filter(Boolean).join(' ').toLowerCase()}">
        ...
        <div class="card-body">
          ...
          <div class="brand-tags">
            ...
            ${b.macroLabel ? `<span class="badge zone-badge">${b.macroLabel}</span>` : ''}
          </div>
          <p class="xpark-caption brand-floor mono">${b.zone || b.floor} · ${b.area}</p>
  ...
  function applyFilters() {
    document.querySelectorAll('.brand-card').forEach((card) => {
      ...
      const matchZone = activeZone === '全部' || zone === activeZone;
      const q = searchQuery;
      const matchSearch = !q || searchText.includes(q);
      card.classList.toggle('is-hidden', !(matchTier && matchCat && matchZone && matchSearch));
    });
    updateCount();
  }
